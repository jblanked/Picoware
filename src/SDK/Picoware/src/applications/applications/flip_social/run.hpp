...
class FlipSocialRun
{
    ...
    std::unique_ptr<HTTP> http;            // HTTP instance for making requests
    bool inputHeld;                        // flag to check if input is held
    int lastInput;                         // last input key pressed
    ...
    ViewManager *viewManager;              // View manager instance
    //
    ...
    void drawFeedItem(Canvas *canvas, String username, String message, String flipped, String flips, String date_created); // draw a single feed item
...
public:
    FlipSocialRun(ViewManager *ViewManager);
    ~FlipSocialRun();
    //
    ...
    void updateInput(int input);                                                                     // update input for the run
};

static FlipSocialRun *flipSocialRunInstance = nullptr;
static uint16_t flipSocialRunOriginalColor = 0;
static bool flipSocialRunStart(ViewManager *viewManager)
{
    // Create a new FlipSocialRun instance
    flipSocialRunInstance = new FlipSocialRun(viewManager);
    flipSocialRunOriginalColor = viewManager->getBackgroundColor();
    viewManager->setBackgroundColor(TFT_WHITE);
    return flipSocialRunInstance != nullptr;
}

static void flipSocialRunUpdate(ViewManager *viewManager)
{
    if (flipSocialRunInstance && flipSocialRunInstance->isActive())
    {
    ...
    {
        viewManager->back(); // Go back to the previous view
    }
}

static void flipSocialRunStop(ViewManager *viewManager)
{
    viewManager->setBackgroundColor(flipSocialRunOriginalColor);
    delete flipSocialRunInstance;
    flipSocialRunInstance = nullptr;
}

static const View flipSocialRunView = View("FlipSocialRun", flipSocialRunUpdate, flipSocialRunStart, flipSocialRunStop);