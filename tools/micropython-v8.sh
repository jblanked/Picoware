#!/bin/bash
# Script to build Picoware MicroPython firmware for the Marauder V8 (ESP32-C5)
...
v8_src_dir="$picoware_dir/src/MicroPython/V8"
...
echo "Initializing and preparing V8 build environment..."
...
require_file "$v8_src_dir/micropython.cmake"
require_file "$v8_src_dir/mpconfigboard.h"
require_file "$v8_src_dir/partitions.csv"
require_file "$v8_src_dir/sdkconfig.defaults"
...
echo "Cleaning previous V8 outputs and build directory..."
rm -f "$output_dir"/Picoware-V8.bin
rm -f "$output_dir"/Picoware-V8-bootloader.bin
rm -f "$output_dir"/Picoware-V8-partition-table.bin
rm -rf "$build_dir"
...
echo "Installing Picoware and V8 modules into MicroPython ports/esp32/modules..."
cp "$picoware_dir/src/MicroPython/main.py" "$micropython_dir/modules/main.py"

stage_required_module_dir "picoware"

mkdir -p "$micropython_dir/modules/v8"
cp "$v8_src_dir/micropython.cmake" "$micropython_dir/modules/v8/micropython.cmake"
cp "$v8_src_dir/board_config.h" "$micropython_dir/modules/v8/board_config.h"
cp -r "$v8_src_dir/i2c" "$micropython_dir/modules/v8/i2c"
cp -r "$v8_src_dir/lcd" "$micropython_dir/modules/v8/lcd"
cp -r "$v8_src_dir/touch" "$micropython_dir/modules/v8/touch"
cp -r "$v8_src_dir/battery" "$micropython_dir/modules/v8/battery"
cp -r "$v8_src_dir/sd" "$micropython_dir/modules/v8/sd"

echo "Staging shared C modules referenced by V8 CMake..."
...
' "$v8_src_dir/micropython.cmake" | sort -u)"

if [ -z "$shared_c_modules" ]; then
    echo "ERROR: No shared C modules found in $v8_src_dir/micropython.cmake"
    exit 1
fi
...
echo "Copying V8 flash/partition configuration overrides..."
cp "$v8_src_dir/partitions.csv" "$micropython_dir/partitions.csv"
cp "$v8_src_dir/sdkconfig.defaults" "$board_dir/sdkconfig.board"
cp "$v8_src_dir/mpconfigboard.h" "$board_dir/mpconfigboard.h"

echo "Validating V8 partition table layout..."
partition_validation="$(python3 - "$v8_src_dir/partitions.csv" <<'PY'
import sys
...
if [ -z "$partition_validation" ]; then
    echo "ERROR: Failed to parse factory partition size from $v8_src_dir/partitions.csv"
    exit 1
fi
...
if ! grep -q "boards/$board_name/sdkconfig.board" "$board_mpconfig_cmake"; then
    echo "Patching $board_name board CMake to include V8 sdkconfig overrides..."
    cat >> "$board_mpconfig_cmake" <<EOF

# V8 override injected by Picoware build script.
...
echo "Starting V8 firmware build..."
cd "$micropython_dir"

# Keep ESP-IDF warnings from failing the build, keep legacy I2C API checks permissive,
# and force the V8 board define for preprocess-only qstr generation paths.
export EXTRA_CFLAGS="-Wno-maybe-uninitialized -Wno-error=maybe-uninitialized -DCONFIG_I2C_SKIP_LEGACY_CONFLICT_CHECK=1 -DV8 -DPBUF_POOL_SIZE=10 -DESP32"

make BOARD=$board_name \
    USER_C_MODULES="$micropython_dir/modules/v8/micropython.cmake" \
    clean
...
if ! grep -q '^CONFIG_ESPTOOLPY_FLASHSIZE_8MB=y$' "$board_dir/sdkconfig.board" \
    ...
    echo "ERROR: V8 sdkconfig defaults are missing expected flash/partition settings."
...
if [ -f "$effective_sdkconfig" ]; then
    if ! grep -q '^CONFIG_ESPTOOLPY_FLASHSIZE_8MB=y$' "$effective_sdkconfig" \
        ...
        echo "ERROR: V8 flash/partition overrides were not applied."
    ...
    if ! grep -q '^CONFIG_SPIRAM=y$' "$effective_sdkconfig"; then
        echo "ERROR: PSRAM is disabled in the effective sdkconfig, but the V8's"
...
cp "$build_dir/micropython.bin" "$output_dir/Picoware-V8.bin"

if [ -f "$build_dir/bootloader/bootloader.bin" ]; then
    cp "$build_dir/bootloader/bootloader.bin" "$output_dir/Picoware-V8-bootloader.bin"
fi

if [ -f "$build_dir/partition_table/partition-table.bin" ]; then
    cp "$build_dir/partition_table/partition-table.bin" "$output_dir/Picoware-V8-partition-table.bin"
fi

echo "V8 build complete."
echo "Artifacts:"
echo "  $output_dir/Picoware-V8.bin"
if [ -f "$output_dir/Picoware-V8-bootloader.bin" ]; then
    echo "  $output_dir/Picoware-V8-bootloader.bin"
fi
if [ -f "$output_dir/Picoware-V8-partition-table.bin" ]; then
    echo "  $output_dir/Picoware-V8-partition-table.bin"
fi
