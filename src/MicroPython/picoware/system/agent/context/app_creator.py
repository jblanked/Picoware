...
Use `storage_write` with mode `w` for Python source. If it returns false, correct
the path or request and retry only once when the correction is meaningful. After
writing, validate and read the file back to verify that the complete source was
saved.
""")
