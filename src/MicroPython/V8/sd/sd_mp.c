...
static inline mp_v8_file_obj_t *sd_mp_file_from_obj(mp_obj_t self_in)
{
    return (mp_v8_file_obj_t *)MP_OBJ_TO_PTR(self_in);
}
...
static void sd_mp_refresh_file_metadata(mp_v8_file_obj_t *self)
{
    struct stat st = {0};
...
static void sd_mp_close_file_handle(mp_v8_file_obj_t *self)
{
    if (self == NULL)
    {
        return;
...
static bool sd_mp_open_file_handle(mp_v8_file_obj_t *self, const char *mode, bool create_if_missing)
{
...
mp_obj_t mp_fat32_file_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
    ...
    mp_v8_file_obj_t *self = mp_obj_malloc_with_finaliser(mp_v8_file_obj_t, &mp_fat32_file_type);
...
{
    mp_v8_file_obj_t *file = sd_mp_file_from_obj(file_obj);
...
{
    char path[SD_MP_PATH_MAX];
    mp_v8_file_obj_t *file_obj = NULL;
    ...
    file_obj = (mp_v8_file_obj_t *)MP_OBJ_TO_PTR(mp_fat32_file_make_new(&mp_fat32_file_type, 0, 0, NULL));
