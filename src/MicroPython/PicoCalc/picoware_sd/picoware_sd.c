...
STATIC mp_obj_t picoware_sd_close(mp_obj_t file_obj)
{
    mp_fat32_file_obj_t *file = MP_OBJ_TO_PTR(file_obj);
    fat32_close(&file->file);
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(picoware_sd_close_obj, picoware_sd_close);
...
STATIC mp_obj_t picoware_sd_open(mp_obj_t filepath_obj)
{
    const char *filePath = mp_obj_str_get_str(filepath_obj);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(picoware_sd_open_obj, picoware_sd_open);
...
STATIC const mp_rom_map_elem_t picoware_sd_module_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_close), MP_ROM_PTR(&picoware_sd_close_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_open), MP_ROM_PTR(&picoware_sd_open_obj)},
