...
void picoware_lvgl_choice_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
    (void)kind;
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_print_str(print, "Choice(state=");
    mp_print_str(print, "state=");
    mp_obj_print_helper(print, mp_obj_new_int(self->state), PRINT_REPR);
    mp_print_str(print, ", title=");
    if (self->title_label)
    {
        const char *title = lv_label_get_text(self->title_label);
        mp_print_str(print, title);
    }
    else
    {
...
// Choice constructor
mp_obj_t picoware_lvgl_choice_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
...
void picoware_lvgl_choice_attr(mp_obj_t self_in, qstr attribute, mp_obj_t *destination)
{
    if (destination[0] == MP_OBJ_NULL)
    {
        ...
        {
        case MP_QSTR_state:
            destination[0] = picoware_lvgl_choice_get_state(self_in);
            break;
        case MP_QSTR_title:
        {
            picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
            ...
            {
                const char *title = lv_label_get_text(self->title_label);
                destination[0] = mp_obj_new_str(title, strlen(title));
            }
            else
            {
        ...
        {
            picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
            destination[0] = mp_obj_new_int(self->foreground_color_565);
            break;
        }
    ...
    else if (destination[1] != MP_OBJ_NULL)
    {
        ...
        {
        case MP_QSTR_state:
            picoware_lvgl_choice_set_state(self_in, destination[1]);
            break;
        default:
            return; // Fail
        };
...
mp_obj_t picoware_lvgl_choice_get_state(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_set_state(mp_obj_t self_in, mp_obj_t value)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_clear(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_draw(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_reset(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_scroll_down(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_scroll_up(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_close(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_is_open(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
mp_obj_t picoware_lvgl_choice_open(mp_obj_t self_in)
{
    picoware_lvgl_choice_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
MP_DEFINE_CONST_OBJ_TYPE(
    ...
    print, picoware_lvgl_choice_print,
    make_new, picoware_lvgl_choice_make_new,
    attr, picoware_lvgl_choice_attr,
