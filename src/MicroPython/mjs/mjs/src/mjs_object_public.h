...
extern "C"
{
  ...
  /*
   * Register a getter callback for a property.
   * When JS reads `obj.name`, the getter is invoked automatically.
   * If `name_len` is ~0, `name` is assumed to be NUL-terminated.
   */
  mjs_err_t mjs_set_getter(struct mjs *mjs, mjs_val_t obj, const char *name,
                           size_t name_len, mjs_val_t (*getter)(struct mjs *));
