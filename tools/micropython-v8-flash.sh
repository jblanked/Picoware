#!/bin/bash
# Flash Picoware MicroPython firmware to the Marauder V8 (ESP32-C5)
...
usage() {
    cat <<'EOF'
Usage:
    bash tools/micropython-v8-flash.sh [--port PORT] [--baud BAUD] [--no-erase] [--no-verify]
...
Environment overrides:
    ...
    V8_BUILD_DIR     Path to V8 build artifacts directory
    V8_PORT          Default serial port
    V8_BAUD          Default baud rate
EOF
}
...
build_dir="${V8_BUILD_DIR:-$picoware_dir/builds/MicroPython}"
...
if [ -z "$port" ]; then
    ...
    echo "Pass --port /dev/ttyUSB0 or set V8_PORT."
    echo
...
bootloader_bin="$build_dir/Picoware-V8-bootloader.bin"
partition_bin="$build_dir/Picoware-V8-partition-table.bin"
firmware_bin="$build_dir/Picoware-V8.bin"

if [ ! -f "$bootloader_bin" ] || [ ! -f "$partition_bin" ] || [ ! -f "$firmware_bin" ]; then
    echo "ERROR: V8 flash artifacts were not found in $build_dir"
    echo "Expected files:"
    echo "  - $bootloader_bin"
    echo "  - $partition_bin"
    echo "  - $firmware_bin"
    echo "Build first:"
    echo "  bash tools/micropython-v8.sh"
    exit 1
fi
