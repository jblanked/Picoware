...
} mp_pancake_file_obj_t;

static inline mp_pancake_file_obj_t *sd_mp_file_from_obj(mp_obj_t self_in)
{
    return (mp_pancake_file_obj_t *)MP_OBJ_TO_PTR(self_in);
}
...
static void sd_mp_refresh_file_metadata(mp_pancake_file_obj_t *self)
{
    struct stat st = {0};
...
static void sd_mp_close_file_handle(mp_pancake_file_obj_t *self)
{
    if (self == NULL)
    {
        return;
...
static bool sd_mp_open_file_handle(mp_pancake_file_obj_t *self, const char *mode, bool create_if_missing)
{
...
{
    (void)kind;
    mp_pancake_file_obj_t *self = sd_mp_file_from_obj(self_in);
...
mp_obj_t mp_fat32_file_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args)
{
    ...
    mp_pancake_file_obj_t *self = mp_obj_malloc_with_finaliser(mp_pancake_file_obj_t, &mp_fat32_file_type);
...
{
    mp_pancake_file_obj_t *file = sd_mp_file_from_obj(file_obj);
...
{
    mp_pancake_file_obj_t *source = NULL;
    char destination_path[SD_MP_PATH_MAX];
    struct stat st = {0};
    size_t bytes_per_chunk = SD_MP_COPY_CHUNK_DEFAULT;
...
{
    char path[SD_MP_PATH_MAX];
    mp_pancake_file_obj_t *file_obj = NULL;
    ...
    file_obj = (mp_pancake_file_obj_t *)MP_OBJ_TO_PTR(mp_fat32_file_make_new(&mp_fat32_file_type, 0, 0, NULL));
