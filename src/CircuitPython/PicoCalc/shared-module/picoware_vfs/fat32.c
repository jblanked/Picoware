...
static fat32_error_t link_entry(fat32_entry_t *entry, const char *path)
{
    ...
    {
        filename = path_copy;
        parent_path = path_copy; // Point to empty string at start
        parent_path[0] = '\0';
    }
...
fat32_error_t fat32_write(fat32_file_t *file, const void *buffer, size_t size, size_t *bytes_written)
{
    ...
    if (file->file_size < old_file_size)
    {
        ...
        uint32_t new_needed_clusters = (file->file_size == 0) ? 0 : (file->file_size + bytes_per_cluster - 1) / bytes_per_cluster;
        uint32_t old_current_clusters = (old_file_size == 0) ? 0 : (old_file_size + bytes_per_cluster - 1) / bytes_per_cluster;

        if (new_needed_clusters < old_current_clusters && file->start_cluster >= 2)
        {
            uint32_t last_cluster_to_keep = file->start_cluster;
            if (new_needed_clusters > 0)
            {
                // Seek to last cluster to keep
                fat32_error_t seek_res = seek_to_cluster(file->start_cluster, new_needed_clusters - 1, &last_cluster_to_keep);
