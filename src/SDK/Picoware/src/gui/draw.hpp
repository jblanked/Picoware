...
public:
    ...
    void imageBitmap(                                                                    // Draws a 1-bit packed bitmap on the display at the specified position with the specified color.
        Vector position,                                                                 // position
        const uint8_t *bitmap,                                                           // data (1-bit packed)
        Vector size,                                                                     // size of image
        uint16_t color,                                                                  // color
        bool invert = false);                                                            // if false, set bits are drawn, if true, clear bits are drawn
    void imageColor(                                                                     // Draws a bitmap on the display at the specified position with the specified color.
        Vector position,                                                                 // position
        const uint8_t *bitmap,                                                           // data
        Vector size,                                                                     // size of image
        uint16_t color,                                                                  // color
        bool invert = false,                                                             // if false, non-transparent colors are set to color otherwise transparent colors are set to color
...
private:
    ...
    bool useBackgroundTextColor;                              // Use background color for text rendering
};
