...
extern "C"
{
  ...
  enum mjs_opcode
  {
    ...
    OP_SYNC_SCOPE,        /* ( val -- val ) scope[name] = val, operand: name string */
    OP_ENTER_FAST,        /* ( -- ) Push fast_locals frame marker */
    OP_EXIT_FAST,         /* ( -- ) Pop fast_locals frame, truncate */
    OP_LOAD_FAST,         /* ( -- val ) Push fast_locals[index], operand: varint abs_index */
    OP_STORE_FAST,        /* ( val -- ) Pop to fast_locals[index], operand: varint abs_index */
    OP_INC_FAST,          /* ( -- old_val ) fast_locals[index]++, postfix, operand: varint abs_index */
    OP_DEC_FAST,          /* ( -- old_val ) fast_locals[index]--, postfix, operand: varint abs_index */
    OP_MAX
  };
