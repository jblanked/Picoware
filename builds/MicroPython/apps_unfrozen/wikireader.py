...
settings = {"full_article": True, "language": "en", "theme": "system", "text_margin": 10, "history": [], "favorites": [], "offline": []}
...
def _remove_latex(text):
    ...
    for target in ("{\\displaystyle", "{\\textstyle"):
        ...
        while True:
            ...
            # Guardian RAM Rules: Native .find() prevents massive 1-byte string allocations
...
def change_state(view_manager, new_state):
    ...
    if current_state == STATE_VIEW_ARTICLE and article_textbox:
        ...
        keep_article = new_state in (STATE_FIND_KEYBOARD, STATE_HELP, STATE_QUICK_ACTIONS, STATE_LANGUAGES) or (new_state == STATE_LIST_VIEW and current_list_type in ("toc", "links"))
...
def _add_to_history(view_manager):
    ...
    history.insert(0, [current_article_page_id, current_article_title, -1])
...
def _toggle_favorite(view_manager):
    ...
    if removed:
        ...
        favorites.insert(0, [current_article_page_id, current_article_title, -1])
    ...
    try:
        if removed:
            ...
            if view_manager.storage.exists(cache_path) or view_manager.storage.exists(legacy_cache):
                ...
                offline.insert(0, [current_article_page_id, current_article_title, -1])
                while len(offline) > 50:
                    ...
                    if view_manager.storage and view_manager.has_sd_card:
                        ...
                        popped_path_new = f"picoware/wikireader/offline_{lang}_{popped[0]}_{encoded}.json"
                ...
                if escape:
                    ...
                    if c == 110: # b'n'
                        ...
                        if len(paragraphs) % 20 == 0:
                            gc.collect()
                    elif c == 114: extracted.extend(b'\r') # b'r'
            ...
            if first_char not in ('{', '['):
                ...
                if crlf != -1 and (crlf - start_idx) <= 10:
                    try:
                        ...
                        # Pre-allocate exact max buffer to prevent dynamic bytearray fragmentation
                        total_len = len(raw_bytes)
                        decoded = bytearray(total_len)
                        write_idx = 0
                        idx = 0
                        ...
                        while idx < total_len:
                            ...
                            if end <= total_len:
                                decoded[write_idx:write_idx+chunk_size] = raw_bytes[start:end]
                                write_idx += chunk_size
                            idx = end + 2

                        raw_text = decoded[:write_idx].decode('utf-8')
                        del raw_bytes
                        del decoded
                        gc.collect()
                    except ValueError:
                ...
                if _current_request_type == REQ_PRELOAD_LINKS:
                    ...
                    if article_textbox:
                        ...
                        for p_idx, p in enumerate(article_textbox.paragraphs):
                            ...
                            if found_in_this_para and p_idx < len(article_textbox.para_has_links):
                                article_textbox.para_has_links[p_idx] = 1
                            ...
                            if not links_to_check:
                                break
                        del links_to_check
                    article_textbox._update_token_flags()
...
def fetch_links_preload(view_manager, page_id):
    ...
    lang = settings["language"]
    encoded_title = url_encode(current_article_title)
    url = f"{WIKI_API_URL.format(lang=lang)}?action=query&prop=links&titles={encoded_title}&redirects=1&pllimit=250&format=json"
    _reset_http_response()
    _request_start_time = ticks_ms()
    if not http_client.get_async(url, headers=WIKI_HEADERS, timeout=20):
...
def fetch_article(view_manager, page_id):
    ...
    if view_manager.storage and view_manager.has_sd_card:
        try:
            lang = settings["language"]
            encoded_title = url_encode(current_article_title)
            path_offline = f"picoware/wikireader/offline_{lang}_{page_id}_{encoded_title}.json"
            path_cache = f"picoware/wikireader/cache_{lang}_{page_id}_{encoded_title}.json"
            path_legacy_offline = f"picoware/wikireader/offline_{page_id}_{encoded_title}.json"
            path_legacy_cache = f"picoware/wikireader/cache_{page_id}_{encoded_title}.json"
            path_legacy = f"picoware/wikireader/cache_{page_id}.json"

            if view_manager.storage.exists(path_offline):
                cache_path = path_offline
            elif view_manager.storage.exists(path_legacy_offline):
                cache_path = path_legacy_offline
            elif view_manager.storage.exists(path_cache):
                cache_path = path_cache
            elif view_manager.storage.exists(path_legacy_cache):
                cache_path = path_legacy_cache
    ...
    url = f"{WIKI_API_URL.format(lang=lang)}?action=query&prop=extracts&explaintext=true&redirects=1&pageids={page_id}{exintro}&format=json"
...
class CustomArticleViewer:
    ...
    def __init__(self, draw, y, height, text_color, bg_color, theme_color):
        ...
        self.para_has_links = bytearray()

        # Force base font size due to LVGL scaling bug
        self.font_size = 0
        font = draw.get_font(0)
        self.char_w = font.width + font.spacing
    ...
    def set_text(self, text_data):
        ...
        if isinstance(text_data, str):
            ...
            while start_idx < text_len:
                ...
                if end_idx < text_len:
                    self.paragraphs.append(para_str.strip() + '\n')
                elif para_str.strip():
                    self.paragraphs.append(para_str.strip())
    ...
    def _tokenize_and_wrap(self):
        global loading_spinner
        # Guardian RAM Rules: Clean up previous structures before re-allocating
        del self.para_tokens[:]
        self.line_indices = bytearray()
        self.cached_toc = bytearray()
        self.num_lines = 0
        self.para_has_links = bytearray(max(1, len(self.paragraphs)))
        gc.collect()

        # Pre-allocate static bytearray for line indices (smaller initial size to prevent out-of-memory errors)
        self.line_indices = bytearray(256 * 6)
        line_offset = 0

        # Guardian RAM Rules: Pre-allocate static scratchpad to prevent .extend() fragmentation per word
        token_buffer = bytearray(512 * 4)

        margin = settings.get("text_margin", 10)
        # Symmetrical margins: account for left/right padding, scrollbar (4px), and proportional overhangs (8px)
        max_w = self.draw.size.x - (margin * 2) - 12
        ...
        for p_idx, para in enumerate(paragraphs):
            ...
            if para.startswith("=="):
                ...
                if p_strip.endswith("=="):
                    ...
                    # Guardian RAM Rules: Inject blank line BEFORE heading
                    if line_offset + 6 > len(self.line_indices):
                        self.line_indices.extend(bytearray(256 * 6))
                    self.line_indices[line_offset:line_offset+6] = bytes([p_idx_lo, p_idx_hi, 0, 0, 0, 0])
                    line_offset += 6
                    self.num_lines += 1

                    self.cached_toc.extend(bytes([p_idx_lo, p_idx_hi, self.num_lines & 0xFF, (self.num_lines >> 8) & 0xFF]))
        ...
        # Tokenizer Phase
            while i < text_len:
                ...
                start = i
                # Guardian RAM Rules: O(1) Word Leaping using native C string search
                space_idx = para.find(' ', start)
                nl_idx = para.find('\n', start)
                cr_idx = para.find('\r', start)
                end = text_len
                if space_idx != -1 and space_idx < end: end = space_idx
                if nl_idx != -1 and nl_idx < end: end = nl_idx
                if cr_idx != -1 and cr_idx < end: end = cr_idx
                i = end
                length = i - start
                w = length * self.char_w

                # Handle Force-Breaks for massively long words
                if w > max_w and length > 1:
                    sub_start = start
                    while sub_start < i:
                        sub_len = 1
                        while sub_start + sub_len < i and ((sub_len + 1) * self.char_w) <= max_w:
                            sub_len += 1
                        if sub_len > 1: sub_len -= 1

                        is_last = (sub_start + sub_len >= i)

                        flags = 0
                        if not is_last:
                            flags |= 0x2000 # Bit 13: Hyphen Flag

                        if tok_offset + 4 > len(token_buffer):
                            token_buffer.extend(bytearray(256 * 4))
                        token_buffer[tok_offset:tok_offset+2] = sub_start.to_bytes(2, 'little')
                        token_buffer[tok_offset+2:tok_offset+4] = (flags | sub_len).to_bytes(2, 'little')
                        tok_offset += 4
                        sub_start += sub_len
                else:
                    if tok_offset + 4 > len(token_buffer):
                        token_buffer.extend(bytearray(256 * 4))
                    token_buffer[tok_offset:tok_offset+2] = start.to_bytes(2, 'little')
                    token_buffer[tok_offset+2:tok_offset+4] = length.to_bytes(2, 'little')
                    tok_offset += 4

            tokens = token_buffer[:tok_offset]
            self.para_tokens.append(tokens)

            # Wrapping Phase
            num_tokens = tok_offset // 4
            ...
            while tok_idx < num_tokens:
                line_start_tok = tok_idx
                idx = tok_idx * 4
                first_tok_start = tokens[idx] | (tokens[idx+1] << 8)
                current_x = (first_tok_start * self.char_w) if line_start_tok == 0 else 0

                line_end_tok = tok_idx
                while tok_idx < num_tokens:
                    idx = tok_idx * 4
                    t_start = tokens[idx] | (tokens[idx+1] << 8)
                    t_len_flags = tokens[idx+2] | (tokens[idx+3] << 8)
                    t_len = t_len_flags & 0x1FFF
                    t_w = t_len * self.char_w

                    eff_w = t_w + (self.char_w if (t_len_flags & 0x2000) else 0)

                    if tok_idx > line_start_tok:
                        prev_idx = (tok_idx - 1) * 4
                        p_start = tokens[prev_idx] | (tokens[prev_idx+1] << 8)
                        p_len = (tokens[prev_idx+2] | (tokens[prev_idx+3] << 8)) & 0x1FFF
                        # Guardian RAM Rules: O(1) integer math replaces memory-heavy string duplication
                        gap_px = (t_start - (p_start + p_len)) * self.char_w

                        if current_x + gap_px + eff_w > max_w:
                            break
                        current_x += gap_px
                ...
                if line_offset + 6 > len(self.line_indices):
                    self.line_indices.extend(bytearray(256 * 6))
                self.line_indices[line_offset] = p_idx_lo
                self.line_indices[line_offset+1] = p_idx_hi
                self.line_indices[line_offset+2] = line_start_tok & 0xFF
                self.line_indices[line_offset+3] = (line_start_tok >> 8) & 0xFF
                self.line_indices[line_offset+4] = line_end_tok & 0xFF
                self.line_indices[line_offset+5] = (line_end_tok >> 8) & 0xFF
                line_offset += 6
                self.num_lines += 1

            if is_heading:
                # Guardian RAM Rules: Inject blank line AFTER heading
                if line_offset + 6 > len(self.line_indices):
                    self.line_indices.extend(bytearray(256 * 6))
                self.line_indices[line_offset:line_offset+6] = bytes([p_idx_lo, p_idx_hi, 0, 0, 0, 0])
                line_offset += 6
        ...
        self.line_indices = self.line_indices[:line_offset]

        self.top_line = 0

        del token_buffer
        gc.collect()
        self._update_token_flags()
        return True

    def _update_token_flags(self):
        """Applies dynamic layout attribute bitmasks directly to the bytearray."""
        global loading_spinner
        term = self.last_search_term.lower() if self.last_search_term else None
        links = self.inline_links
        has_para_links = hasattr(self, 'para_has_links') and bool(self.para_has_links)

        total_paras = max(1, len(self.para_tokens))
        last_update_time = ticks_ms()
        vm = get_view_manager()

        for p_idx, tokens in enumerate(self.para_tokens):
            now = ticks_ms()
            if ticks_diff(now, last_update_time) > 150:
                last_update_time = now
                pct = int((p_idx / total_paras) * 100)
                if loading_spinner:
                    if vm:
                        vm.draw.fill_screen(vm.background_color)
                    loading_spinner.text = f"Highlighting... {pct}%"
                    loading_spinner.animate()
                    if vm:
                        vm.draw.swap()
            ...
            para_has_link = has_para_links and p_idx < len(self.para_has_links) and self.para_has_links[p_idx]

            if term or (links and para_has_link):
                para = self.paragraphs[p_idx].lower()

                if term:
                    idx = 0
                    t_len = len(term)
                    while True:
                        idx = para.find(term, idx)
                        if idx == -1: break
                        search_ranges.append((idx, idx + t_len))
                        idx += t_len

                if links and para_has_link:
                    for lnk in links:
                        idx = 0
                        l_len = len(lnk)
                        while True:
                            idx = para.find(lnk, idx)
                            if idx == -1: break
                            link_ranges.append((idx, idx + l_len))
                            idx += l_len
                    link_ranges.sort()
            ...
            s_idx = 0
            s_len = len(search_ranges)
            l_idx = 0
            l_len = len(link_ranges)

            for i in range(num_tokens):
                idx = i * 4
                start = tokens[idx] | (tokens[idx+1] << 8)
                t_len_flags = tokens[idx+2] | (tokens[idx+3] << 8)
                length = t_len_flags & 0x1FFF
                end = start + length
                flags = t_len_flags & 0x2000 # keep hyphen flag

                # Guardian RAM Rules: O(T+R) Two-Pointer overlap check completely bypasses nested O(T*R) loop
                while s_idx < s_len and search_ranges[s_idx][1] <= start:
                    s_idx += 1
                if s_idx < s_len and search_ranges[s_idx][0] < end:
                    flags |= 0x8000

                while l_idx < l_len and link_ranges[l_idx][1] <= start:
                    l_idx += 1
                if l_idx < l_len and link_ranges[l_idx][0] < end:
                    flags |= 0x4000

                tokens[idx+2:idx+4] = (flags | length).to_bytes(2, 'little')
    ...
    def find_term(self, term):
        ...
        if self.last_search_term != term:
            ...
            for p_idx in range(len(self.paragraphs)):
                para_lower = self.paragraphs[p_idx].lower()
                self.search_match_count += para_lower.count(term_lower)
                del para_lower
                if p_idx % 10 == 0:
                    gc.collect()
        ...
        if self.top_line < self.num_lines:
            ...
            start_p_idx = self.line_indices[offset] | (self.line_indices[offset+1] << 8)

        target_p_idx = -1
        # Search forward
        for p_idx in range(start_p_idx + 1, len(self.paragraphs)):
            para_lower = self.paragraphs[p_idx].lower()
            found = term_lower in para_lower
            del para_lower
            if found:
                target_p_idx = p_idx
                break
        ...
        for i in range(self.num_lines):
            offset = i * 6
            p_idx = self.line_indices[offset] | (self.line_indices[offset+1] << 8)
    ...
    def get_visible_text(self):
        ...
        start_tok = self.line_indices[offset_start+2] | (self.line_indices[offset_start+3] << 8)
        ...
        end_tok = self.line_indices[offset_end+4] | (self.line_indices[offset_end+5] << 8)

        start_char = 0
        if start_tok > 0:
            t_idx = start_tok * 4
            start_char = self.para_tokens[start_p][t_idx] | (self.para_tokens[start_p][t_idx+1] << 8)

        end_char = len(self.paragraphs[end_p])
        if end_tok > 0:
            t_idx = (end_tok - 1) * 4
            e_start = self.para_tokens[end_p][t_idx] | (self.para_tokens[end_p][t_idx+1] << 8)
            e_len = (self.para_tokens[end_p][t_idx+2] | (self.para_tokens[end_p][t_idx+3] << 8)) & 0x1FFF
            end_char = e_start + e_len
    ...
    def draw_viewer(self, title):
        ...
        max_chars = avail_w // max(1, char_w0)
        req_chars = len(disp_title) + len(heading_str) + len(match_str)
        if req_chars > max_chars:
            ...
            if len(disp_title) > excess + 3:
                ...
                rem_req = len(heading_str) + len(match_str)
        ...
        if self.inline_links:
            if self.links_preloaded:
                ...
                for link in self.inline_links:
                    ...
                    if link_lower in vis_text:
                        active_links.append(link_lower)
                del vis_text
                gc.collect()
        ...
        for i in range(self.visible_lines):
            ...
            if line_idx < total_lines:
                ...
                start_tok = line_indices[offset+2] | (line_indices[offset+3] << 8)
                end_tok = line_indices[offset+4] | (line_indices[offset+5] << 8)

                if start_tok < end_tok:
                    tokens = self.para_tokens[p_idx]
                    ...
                    for t_idx in range(start_tok, end_tok):
                        idx = t_idx * 4
                        t_start = tokens[idx] | (tokens[idx+1] << 8)
                        t_len_flags = tokens[idx+2] | (tokens[idx+3] << 8)
                        t_len = t_len_flags & 0x1FFF
                        flags = t_len_flags & 0xE000
                        t_w = t_len * self.char_w
                        ...
                        if flags & 0x2000:
                            draw_word = para[t_start:t_start+t_len] + "-"
                            bg_w += self.char_w
                            eff_w += self.char_w
                        else:
                            # Guardian RAM Rules: Slice the trailing space natively to bypass bounding box truncation
                            if t_start + t_len < len(para) and para[t_start + t_len] == ' ':
                                draw_word = para[t_start:t_start+t_len+1]
                                bg_w += self.char_w
                            else:
                                draw_word = para[t_start:t_start+t_len] + " "
                                bg_w += self.char_w

                        # Mathematically guarantee spaces exactly as the wrapper planned
                        if t_idx > start_tok:
                            prev_idx = (t_idx - 1) * 4
                            p_start = tokens[prev_idx] | (tokens[prev_idx+1] << 8)
                            p_len = (tokens[prev_idx+2] | (tokens[prev_idx+3] << 8)) & 0x1FFF
                        ...
                        if flags & 0x8000:
                            v_pos.y = y_pos - 1
                            v_size.x = bg_w; v_size.y = l_height - 2
                            d_fill(v_pos, v_size, th_col)
                            v_pos.y = y_pos
                            d_text(v_pos, draw_word, bg_col, f_size)
                        elif flags & 0x4000:
                            v_pos.y = y_pos - 1
                            v_size.x = bg_w; v_size.y = l_height - 2
                            d_fill(v_pos, v_size, bg_col)
                            v_pos.y = y_pos
                            d_text(v_pos, draw_word, lnk_col, f_size)
                        else:
                            v_pos.y = y_pos
                            d_text(v_pos, draw_word, fg_col, f_size)
            ...
            for i, item in enumerate(items):
                ...
                if storage and has_sd:
                    try:
                        ...
                        if (f"offline_{lang}_{pageid}_{encoded}.json" in local_files or
                            f"cache_{lang}_{pageid}_{encoded}.json" in local_files or
...
def setup_quick_actions(view_manager):
    ...
    active_ui.add_item("Find in Page")
...
def run_view_article(view_manager):
    ...
    global language_menu_origin_state, article_textbox, current_article_title, current_article_page_id, help_origin_state, links_data, _current_request_type, loading_spinner, _is_random_article, current_list_type
...
def run_settings(view_manager):
    ...
    if button == BUTTON_UP:
        ...
        if selected_index == 0:  # Full Article
            ...
            _settings_dirty = True
            _refresh_settings_menu_items(view_manager)
        elif selected_index == 5:  # Clear Data...
            change_state(view_manager, STATE_CLEAR_DATA)
        elif selected_index == 6:  # Save and Back
            save_settings_to_sd(view_manager)
            change_state(view_manager, STATE_MAIN_MENU)
...
def run_languages(view_manager):
    ...
    if button == BUTTON_UP:
        ...
        if language_menu_origin_state == STATE_VIEW_ARTICLE and new_lang != old_lang:
            ...
            encoded_title = url_encode(current_article_title)
            url = f"{WIKI_API_URL.format(lang=old_lang)}?action=query&prop=langlinks&lllang={new_lang}&redirects=1&titles={encoded_title}&format=json"
            _reset_http_response()
            _request_start_time = ticks_ms()
...
def run(view_manager):
    ...
    if view_manager.input_manager.button == -1 and current_state not in (STATE_LOADING, STATE_SEARCH_KEYBOARD, STATE_FIND_KEYBOARD) and not needs_redraw:
        return
