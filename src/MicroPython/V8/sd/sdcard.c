/*
 * SD card mounting for the Marauder V8.
 *
 * Mounts through machine.SDCard as on the Cardputer, with a POSIX bridge over it
 * at /sdcard for the C modules that use fopen().
 *
 * The panel and the card share one SPI bus, and machine.SDCard raises if the bus
 * is already up, so ordering matters: ViewManager builds Storage before Draw, so
 * this runs first and owns the bus and the display attaches to it. The bus is
 * created with max_transfer_sz = 4000 (see LCD_SWAP_LINES).
 */
...
// Slot 2 is SPI; on this chip that resolves to SPI2_HOST, the panel's bus.
...
esp_err_t sdcard_mount(void)
{
    ...
    if (nlr_push(&nlr) == 0)
    {
        mp_obj_t ctor_args[] = {
            ...
            MP_OBJ_NEW_SMALL_INT(V8_SD_MISO_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_mosi),
            MP_OBJ_NEW_SMALL_INT(V8_SD_MOSI_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_sck),
            MP_OBJ_NEW_SMALL_INT(V8_SD_SCLK_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_cs),
            MP_OBJ_NEW_SMALL_INT(V8_SD_CS_GPIO),
