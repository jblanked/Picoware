...
def __box(draw, x, y, width, height, color):
    ...
    # An undamaged facade needs one native rectangle, not one per terrain row.
    if all(cells):
        if end_x > x and end_y > y:
            draw.fill_rect(x, y, end_x - x, end_y - y, color)
        return
    active = {}
    while y < end_y:
        current = {}
        ...
        while xx < end_x:
            ...
            if solid:
                # Keep vertically adjacent, equal-width runs as one rectangle.
                run = (xx, right)
                current[run] = active.pop(run, y)
            xx = right
        for (left, right), top in active.items():
            draw.fill_rect(left, top, right - left, y - top, color)
        active = current
        y = bottom
    for (left, right), top in active.items():
        draw.fill_rect(left, top, right - left, y - top, color)
