...
char *get_json_value(const char *key, const char *json_data)
{
    ...
    if (json_data != NULL)
    {
        ...
        {
            printf("Insufficient memory for JSON tokens.\n");
            return NULL;
        }
        ...
        {
            printf("Root element is not an object.\n");
            free(tokens);
            return NULL;
        }
...
char *get_json_array_value(const char *key, uint32_t index, const char *json_data)
{
    ...
    if (array_str == NULL)
    {
        printf("Failed to get array for key: %s\n", key);
        return NULL;
    }
    ...
    if (ret < 0)
    {
        printf("Failed to parse JSON array: %d\n", ret);
        free(tokens);
        free(array_str);
        return NULL;
    }
    ...
    {
        printf("Value for key '%s' is not an array.\n", key);
        free(tokens);
        free(array_str);
        return NULL;
    }
    ...
    {
        // printf("Index %lu out of bounds for array with size %u.", index, tokens[0].size);
        free(tokens);
        free(array_str);
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < index; i++)
    {
        ...
        {
            printf("Error skipping tokens to reach element %lu.\n", i);
            free(tokens);
            free(array_str);
            return NULL;
        }
...
char **get_json_array_values(const char *key, const char *json_data, int *num_values)
{
    ...
    if (values == NULL)
    {
        printf("Failed to allocate memory for array of values.\n");
        free(tokens);
        free(array_str);
        return NULL;
    }
    ...
    for (int i = 0; i < array_size; i++)
    {
        ...
        {
            printf("Unexpected end of tokens while traversing array.\n");
            break;
        }
        ...
        {
            printf("Array element %d is not an object, skipping.\n", i);
            // Skip this element
            current_token += 1;
            continue;
        }
