...
void lcd_init(void)
{
    ...
    lcd_write_cmd(0xE4); // gate control: drive all 320 gate lines
    lcd_write_data(0x25);
    lcd_write_data(0x00);
    lcd_write_data(0x00);
...
void lcd_swap(void)
{
    ...
    // set the Y coordinates (shifted by the panel's offset into controller GRAM)
    const uint16_t y_start = LCD_Y_OFFSET;
    const uint16_t y_end = LCD_Y_OFFSET + LCD_HEIGHT - 1;
    lcd_write_cmd(0x2B);
    lcd_write_data((y_start >> 8) & 0xFF);
    lcd_write_data(y_start & 0xFF);
    lcd_write_data((y_end >> 8) & 0xFF);
