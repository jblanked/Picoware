...
#include "jsmn.h"
#ifdef JSMN_LOG_INCLUDE
#include JSMN_LOG_INCLUDE
#endif
...
char *get_json(const char *key, const char *value)
{
    ...
    char *result = (char *)JSMN_MALLOC(length * sizeof(char)); // Allocate memory
    if (result == NULL)
    {
...
char *get_json_value(const char *key, const char *json_data)
{
    ...
    if (json_data != NULL)
    {
        ...
        {
            JSMN_LOG_INFO("JSMM.H", "Insufficient memory for JSON tokens.");
            return NULL;
        }
        // Allocate tokens array on the heap
        jsmntok_t *tokens = JSMN_MALLOC(sizeof(jsmntok_t) * max_tokens);
        if (tokens == NULL)
        {
            JSMN_LOG_INFO("JSMM.H", "Failed to allocate memory for JSON tokens.");
            return NULL;
        }
        ...
        if (ret < 0)
        {
            ...
            JSMN_LOG_INFO("JSMM.H", "Failed to parse JSON: %d", ret);
            JSMN_FREE(tokens);
            return NULL;
        }
        ...
        {
            JSMN_LOG_INFO("JSMM.H", "Root element is not an object.");
            JSMN_FREE(tokens);
            return NULL;
        }
        ...
        for (int i = 1; i < ret; i++)
        {
            if (jsoneq(json_data, &tokens[i], key) == 0)
            {
                ...
                char *value = JSMN_MALLOC(length + 1);
                if (value == NULL)
                {
                    JSMN_LOG_INFO("JSMM.H", "Failed to allocate memory for value.");
                    JSMN_FREE(tokens);
                    return NULL;
                }
                ...
                JSMN_FREE(tokens); // Free the token array
...
char *get_json_array_value(const char *key, uint32_t index, const char *json_data)
{
    ...
    if (array_str == NULL)
    {
        JSMN_LOG_INFO("JSMM.H", "Failed to get array for key: %s", key);
        return NULL;
    }
    ...
    if (ret < 0)
    {
        JSMN_LOG_INFO("JSMM.H", "Failed to parse JSON array: %d", ret);
        JSMN_FREE(tokens);
        JSMN_FREE(array_str);
        return NULL;
    }
    ...
    {
        JSMN_LOG_INFO("JSMM.H", "Value for key '%s' is not an array.", key);
        JSMN_FREE(tokens);
        JSMN_FREE(array_str);
        return NULL;
    }
    ...
    {
        // JSMN_LOG_INFO("JSMM.H", "Index %lu out of bounds for array with size %u.", index, tokens[0].size);
        JSMN_FREE(tokens);
        JSMN_FREE(array_str);
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < index; i++)
    {
        ...
        {
            JSMN_LOG_INFO("JSMM.H", "Error skipping tokens to reach element %lu.", i);
            JSMN_FREE(tokens);
            JSMN_FREE(array_str);
            return NULL;
        }
    ...
    if (!value)
    {
        JSMN_LOG_INFO("JSMM.H", "Failed to allocate memory for array element.");
        JSMN_FREE(tokens);
        JSMN_FREE(array_str);
        return NULL;
    }
...
char **get_json_array_values(const char *key, const char *json_data, int *num_values)
{
    ...
    jsmntok_t *tokens = JSMN_MALLOC(sizeof(jsmntok_t) * max_tokens); // Allocate on the heap
    if (tokens == NULL)
    {
        JSMN_LOG_INFO("JSMM.H", "Failed to allocate memory for JSON tokens.");
        JSMN_FREE(array_str);
    ...
    char **values = JSMN_MALLOC(array_size * sizeof(char *));
    if (values == NULL)
    {
        JSMN_LOG_INFO("JSMM.H", "Failed to allocate memory for array of values.");
        JSMN_FREE(tokens);
        JSMN_FREE(array_str);
        return NULL;
    }
    ...
    for (int i = 0; i < array_size; i++)
    {
        ...
        {
            JSMN_LOG_INFO("JSMM.H", "Unexpected end of tokens while traversing array.");
            break;
        }
        ...
        {
            JSMN_LOG_INFO("JSMM.H", "Array element %d is not an object, skipping.", i);
            // Skip this element
            current_token += 1;
            continue;
        }
