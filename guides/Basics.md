...
### App Store
The App Store requires both a WiFi connection and an SD card. It connects to the Picoware app repository and lets you browse, install, and update third-party apps directly on your device. The main menu has three options: `Update Apps` checks all installed apps for newer versions, `Current App Info` lists your installed apps (press `Left` on an app's detail page to delete it, or `Center` to check for/apply an update), and `Browse App Store` fetches the full catalog from the server. While browsing, use `Up`/`Down` (or `Left`/`Right`) to scroll, `Center` to view an app's details or install it, and `Back` to return to the previous screen. A `Download All` option at the top of the catalog installs every available app in sequence.
