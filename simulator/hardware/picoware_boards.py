...
BOARD_V8 = 13
BOARD_WAVESHARE_1_69_RP2350 = 14
BOARD_DESKTOP = 15
BOARD_PICO_DUO = 16
...
if _name == "picocalc-pico":
    ...
    BOARD_ID = BOARD_V8
elif _name in ("pico-duo", "picoduo"):
    BOARD_ID = BOARD_PICO_DUO
elif _name in ("desktop", "unix"):
    BOARD_ID = BOARD_DESKTOP
elif _name in ("flipper-zero", "flipper"):
    BOARD_ID = BOARD_FLIPPER_ZERO
elif _name in ("waveshare-1.69-rp2350", "waveshare-1-69-rp2350", "waveshare-169-rp2350"):
    BOARD_ID = BOARD_WAVESHARE_1_69_RP2350
else:
...
BOARD_HAS_SD = 0 if BOARD_ID in (BOARD_WAVESHARE_1_28_RP2350, BOARD_WAVESHARE_1_69_RP2350, BOARD_CROWPANEL_10_1, BOARD_PICO_DUO) else 1
BOARD_HAS_TOUCH = 1 if BOARD_ID in (BOARD_WAVESHARE_1_28_RP2350, BOARD_WAVESHARE_1_43_RP2350, BOARD_WAVESHARE_1_69_RP2350, BOARD_WAVESHARE_3_49_RP2350, BOARD_CROWPANEL_10_1, BOARD_WAVESHARE_2_06, BOARD_PANCAKE, BOARD_V8) else 0
BOARD_HAS_WIFI = 1 if BOARD_ID in (BOARD_PICOCALC_PICOW, BOARD_PICOCALC_PICO_2W, BOARD_PICOCALC_PIMORONI_2W, BOARD_CARDPUTER, BOARD_WAVESHARE_2_06, BOARD_PANCAKE, BOARD_V8, BOARD_FLIPPER_ZERO, BOARD_DESKTOP, BOARD_PICO_DUO) else 0
BOARD_HAS_AUDIO = 1 if BOARD_ID in (BOARD_PICOCALC_PICO, BOARD_PICOCALC_PICOW, BOARD_PICOCALC_PICO_2, BOARD_PICOCALC_PICO_2W, BOARD_PICOCALC_PIMORONI_2W, BOARD_DESKTOP) else 0
BOARD_HAS_BLUETOOTH = 1 if BOARD_ID in (BOARD_PICOCALC_PICOW, BOARD_PICOCALC_PICO_2W, BOARD_PICOCALC_PIMORONI_2W, BOARD_CARDPUTER, BOARD_WAVESHARE_2_06, BOARD_PANCAKE, BOARD_V8, BOARD_FLIPPER_ZERO, BOARD_PICO_DUO) else 0
BOARD_HAS_IR = 1 if BOARD_ID in (BOARD_CARDPUTER, BOARD_FLIPPER_ZERO) else 0
BOARD_HAS_IR_RX = 1 if BOARD_ID == BOARD_FLIPPER_ZERO else 0
BOARD_HAS_RP2040 = 1 if BOARD_ID in (BOARD_PICOCALC_PICO, BOARD_PICOCALC_PICOW) else 0
BOARD_HAS_RP2350 = 1 if BOARD_ID in (BOARD_PICOCALC_PICO_2, BOARD_PICOCALC_PICO_2W, BOARD_WAVESHARE_1_28_RP2350, BOARD_WAVESHARE_1_43_RP2350, BOARD_WAVESHARE_1_69_RP2350, BOARD_WAVESHARE_3_49_RP2350, BOARD_PICOCALC_PIMORONI_2W, BOARD_PICO_DUO) else 0
BOARD_HAS_ESP32 = 1 if BOARD_ID in (BOARD_CROWPANEL_10_1, BOARD_CARDPUTER, BOARD_WAVESHARE_2_06, BOARD_PANCAKE, BOARD_V8) else 0
BOARD_HAS_PICOCALC = 1 if BOARD_ID in (BOARD_PICOCALC_PICO, BOARD_PICOCALC_PICOW, BOARD_PICOCALC_PICO_2, BOARD_PICOCALC_PICO_2W, BOARD_PICOCALC_PIMORONI_2W) else 0
BOARD_HAS_KEYBOARD = 1 if BOARD_HAS_PICOCALC or BOARD_ID == BOARD_CARDPUTER else 0
...
def get_device_name():
    ...
    if BOARD_ID == BOARD_PICO_DUO:
        return "Raspberry Pi Pico 2W"
    if BOARD_ID == BOARD_DESKTOP:
        return "Unix Desktop"
...
def has_bluetooth(board_id):
    """Return True if *board_id* has Bluetooth support."""
    return int(board_id) in (
        BOARD_PICOCALC_PICOW,
        BOARD_PICOCALC_PICO_2W,
        BOARD_PICOCALC_PIMORONI_2W,
...
def has_ir(board_id=None):
    """Return True if the board has an infrared transmitter."""
    selected = BOARD_ID if board_id is None else int(board_id)
    return selected in (BOARD_CARDPUTER, BOARD_FLIPPER_ZERO)


def has_ir_rx(board_id=None):
    """Return True if the board has an infrared receiver."""
    selected = BOARD_ID if board_id is None else int(board_id)
    return selected == BOARD_FLIPPER_ZERO


def has_keyboard(board_id):
    """Return True for PicoCalc and Cardputer physical keyboards."""
    return int(board_id) in (
        BOARD_PICOCALC_PICO,
        BOARD_PICOCALC_PICOW,
        BOARD_PICOCALC_PICO_2,
