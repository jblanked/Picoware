...
// The open file objects cannot live in the static array above: gc_collect()
// only scans the stack and registered root pointers, so an mp_obj_t reachable
// only from C statics gets swept while the fd is still open.
// The size must be a literal: root_pointers.h is included from mpstate.h.
MP_REGISTER_ROOT_POINTER(mp_obj_t v8_bridge_open_files[16]);
_Static_assert(BRIDGE_MAX_OPEN_FILES == 16,
               "BRIDGE_MAX_OPEN_FILES must match the MP_REGISTER_ROOT_POINTER array size");

#define BRIDGE_FILE_OBJ(fd) (MP_STATE_PORT(v8_bridge_open_files)[(fd)])
