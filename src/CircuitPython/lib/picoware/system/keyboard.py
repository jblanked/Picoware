...
class Keyboard:
    def __init__(self, scl_pin=GP7, sda_pin=GP6, address=0x1F):
        try:
            print("init keyboard")
            self.hardwarekeyBuf = deque((), 30)
            print("setting I2C")
            try:
                self.i2c = I2C(scl_pin, sda_pin, frequency=10000)
                print("I2C set")
            except Exception as e:
                print("I2C issue: {}".format(e))
                raise (e)
            ...
            if address not in devices:
                print(
                    "Warning: Keyboard device (0x{:02X}) not found on I2C bus".format(
                        address
                    )
                )
                device_list = ["0x{:02X}".format(d) for d in devices]
                print("Available devices:", device_list)
            self.i2c.unlock()
            print("i2c unlocked")
            self.ignor = True
            self.address = address
            print("address set, setting temp")
            self.temp = bytearray(2)
            print("temp set... now resetting")
            self.reset()
            print("reset")
            self.isShift = False
            self.isCtrl = False
    ...
    def enable_report_mods(self):
        ...
        self.write_reg(_REG_CFG, self.temp[0])
    ...
    def readinto(self, buf):
        ...
        # now decide how many keys to send to buf
