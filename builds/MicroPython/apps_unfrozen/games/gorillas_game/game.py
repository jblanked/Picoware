...
def __ai_score(angle, power):
    """Predict one candidate against the same terrain used by real shots."""
    ...
    tx = target.position.x + target.size.x / 2
    ty = target.position.y + target.size.y / 2
    closest = 1e12
    for tick in range(160):
        ...
        for step in range(steps):
            ...
            contact = __contact(x, y, 1 if tick < 6 else -1)
            if contact == 0:
                return 0
            if contact != -2:
                # Prefer impacts near the opponent, opening an obstructed route.
                return distance + (100000 if contact == 1 else 100)
        if x < -20 or x > _state["width"] + 20 or y > _state["baseline"]:
            break
    return closest + _state["width"] ** 2
...
def __ai_update():
    ...
    for candidate in range(2):
        ...
        if index >= 91:
            break
        angle = 20 + (index // 13) * 10
        power = 40 + (index % 13) * 5
        score = __ai_score(angle, power)
        if score < _state["ai_best"][0]:
            _state["ai_best"] = (score, angle, power)
        _state["ai_index"] += 1
    if _state["ai_index"] >= 91 and _state["ai_ticks"] >= 50:
...
def __art(draw, art, x, y, scale, palette, mirror=0, dissolve=0):
    """Render cached pixel runs with a transparent background."""
    for column, row, length, pixel in art:
        ...
        left, top = int(x + offset * scale), int(y + row * scale)
        right, bottom = int(x + (offset + length) * scale), int(y + (row + 1) * scale)
        if right > left and bottom > top:
            __box(draw, left, top, right - left, bottom - top, palette[pixel])
...
def __banana_update(entity, game):
    ...
    for step in range(steps):
        _state["banana_x"] += vx / steps
        _state["banana_y"] += vy / steps
...
def __box(draw, x, y, width, height, color):
    ...
    clip = _state["clip"]
    if clip is None:
        draw.fill_rectangle(Vector(x, y), Vector(width, height), color)
        return
    ...
    if y < by:
        ...
        draw.fill_rectangle(Vector(x, y), Vector(width, top_height), color)
        y += top_height
        height -= top_height
    ...
    while y < end_y:
        ...
        while xx < end_x:
            ...
            if solid:
                draw.fill_rectangle(Vector(xx, y), Vector(right - xx, bottom - y), color)
            xx = right
        y = bottom
...
def __center_text(draw, text, y, color=CREAM):
    ...
    draw.text(Vector(max(0, (_state["width"] - draw.len(text)) // 2), int(y)), text, color)
...
def __damage_terrain(x, y, radius):
    ...
    unit = _state["terrain_unit"]
    outer = radius + 4 * unit
    for bx, by, bw, bh, cols, rows, cells in _state["terrain"]:
...
def __draw_building(entity, draw, game):
    ...
    if index in (1, 5) and __intact_rectangle(x + 2, y + 11, w - 4, _state["font_height"] + 6):
        ...
        draw.text(Vector(x + 4, y + 14), label, neon)
    if index in (0, len(_state["buildings"]) - 1) and __intact_rectangle(x + 5, y + h - 13, 12, 8):
        draw.text(Vector(x + 5, y + h - 13), "01" if index == 0 else "02", TEAL if index == 0 else CORAL)
    __draw_decals(draw, index)
    _state["clip"] = None
...
def __draw_explosion(draw):
    ...
    if age < 13:
        draw.circle(Vector(x, y), max(1, int((4 + age * 1.5) * scale)), WHITE if compact else GOLD)
    ...
    if age >= 10:
        for puff in range(5):
            ...
            if age < 28:
                draw.fill_circle(Vector(px, py), radius, color)
            else:
                draw.circle(Vector(px, py), max(1, radius - (age - 28) // 3), color)
    if age < 22:
        ...
        for lobe in range(6):
            ...
            draw.fill_circle(Vector(px, py), max(1, radius // 2), WHITE if compact else (CORAL if age < 12 else 0xDAA6))
        draw.fill_circle(Vector(x, y), max(1, radius * 2 // 3), 0 if compact else GOLD)
        if age < 9:
            draw.fill_circle(Vector(x, y), max(1, radius // 3), WHITE if compact else CREAM)
...
def __draw_hud(entity, draw, game):
    ...
    if compact:
        ...
        draw.text(Vector(10, 9), "P1", TEAL)
        opponent = "CPU" if _state["players"] == 1 else "P2"
        draw.text(Vector(width - draw.len(opponent) - 10, 9), opponent, CORAL)
        ...
        draw.text(Vector(label_x, 23), turn_label, accent)
        panel_y = _state["baseline"] + 3
        __box(draw, 0, panel_y, width, height - panel_y, INK)
        __box(draw, 8, panel_y, width - 16, 1, SLATE)
        label_y = panel_y + 7
        draw.text(Vector(12, label_y), "ANGLE", SLATE)
        draw.text(Vector(width // 3 + 5, label_y), "POWER", SLATE)
        draw.text(Vector(width * 2 // 3 + 4, label_y), "WIND", SLATE)
        value_y = label_y + font + 3
        draw.text(Vector(12, value_y), "{} DEG".format(_state["angle"]), CREAM)
        draw.text(Vector(width // 3 + 5, value_y), str(_state["power"]), CREAM)
        wind = _state["wind"]
        draw.text(Vector(width * 2 // 3 + 4, value_y), "CALM" if wind == 0 else "{} {}".format("<" if wind < 0 else ">", abs(wind)), TEAL)
...
def __draw_obstacle(entity, draw, game):
    ...
    if kind == 2 and not compact and __terrain_solid(x + w // 2, y):
        for puff in range(3):
            ...
            draw.circle(Vector(int(sx), int(y - 3 - age / 4)), 2 + age // 22, SLATE)
...
def __draw_sky(entity, draw, game):
    ...
    draw.fill_circle(Vector(sun_x, sun_y), radius, 0xCE9D if night else GOLD)
    draw.fill_circle(Vector(sun_x, sun_y - 2), radius - 2, 0xEF7F if night else CREAM)
    surprised = _state["phase"] == PHASE_FLYING and abs(_state["banana_x"] - sun_x) < radius * 2 and abs(_state["banana_y"] - sun_y) < radius * 2
    if night:
        draw.fill_circle(Vector(sun_x - radius // 3, sun_y - 4), max(2, radius // 4), 0xADB9)
        draw.fill_circle(Vector(sun_x + radius // 3, sun_y + 5), max(1, radius // 6), 0xADB9)
    else:
        __box(draw, sun_x - 6, sun_y - 3, 2, 3, INK)
        __box(draw, sun_x + 4, sun_y - 3, 2, 3, INK)
    if surprised and not night:
        draw.circle(Vector(sun_x, sun_y + 5), 3, INK)
    elif not night:
...
def __terrain_solid(x, y):
    """Use a small, persistent material grid for collision and rendering alike."""
    unit = _state["terrain_unit"]
    for bx, by, bw, bh, cols, rows, cells in _state["terrain"]:
...
def run(view_manager):
    ...
    wind = _state["wind"]
    _state["cloud_offset"] += wind * 0.09 if wind else 0.015
    _engine.run_async(True)
