...
STATIC mp_obj_t waveshare_lcd_draw_char(size_t n_args, const mp_obj_t *args)
{
    ...
    {
        mp_raise_ValueError(MP_ERROR_TEXT("draw_char requires 4 arguments: x, y, char, color, and optional font_size"));
    }
    ...
    if (n_args == 5)
    {
        font_size = (FontSize)mp_obj_get_int(args[4]);
        if (font_size < FONT_XTRA_SMALL || font_size > FONT_XTRA_LARGE)
        {
...
STATIC mp_obj_t waveshare_lcd_draw_text(size_t n_args, const mp_obj_t *args)
{
    ...
    if (n_args < 4 || n_args > 5)
    {
        mp_raise_ValueError(MP_ERROR_TEXT("draw_text requires 4 arguments: x, y, text, color, and optional font_size"));
    }
