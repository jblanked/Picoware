...
esp_err_t sdcard_mount(void)
{
    ...
    if (nlr_push(&nlr) == 0)
    {
        mp_obj_t ctor_args[] = {
            ...
            MP_OBJ_NEW_SMALL_INT(WATCH_SD_MISO_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_mosi),
            MP_OBJ_NEW_SMALL_INT(WATCH_SD_MOSI_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_sck),
            MP_OBJ_NEW_SMALL_INT(WATCH_SD_SCLK_GPIO),
            MP_OBJ_NEW_QSTR(MP_QSTR_cs),
            MP_OBJ_NEW_SMALL_INT(WATCH_SD_CS_GPIO),
