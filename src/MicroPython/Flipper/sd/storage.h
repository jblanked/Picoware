...
extern "C"
{
    ...
    static inline size_t storage_read(const char *path, char *buf, size_t len)
    {
        return storage_file_read(path, buf, len);
    }
    static inline bool storage_write(const char *path, const char *data, size_t len, bool overwrite)
    {
        (void)overwrite;
        return storage_file_write(path, data, len);
    }
