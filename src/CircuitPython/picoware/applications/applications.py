    ...
    if button in (BUTTON_UP, BUTTON_LEFT):
        ...
        if selected_app and _app_loader:
            ...
            if view_manager.get_view(app_view_name) is None:
                ...
                print(
                    f"[Applications]: Created view for app {selected_app} after {int(monotonic()) - start_time} ms"
                )
                view_manager.add(app_view)

            view_manager.switch_to(app_view_name)
            print(
                f'[Applications]: Switched to view for app "{selected_app}" after {int(monotonic()) - start_time} ms'
            )
