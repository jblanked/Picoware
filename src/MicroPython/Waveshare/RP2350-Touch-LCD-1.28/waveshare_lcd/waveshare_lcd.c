...
STATIC mp_obj_t waveshare_lcd_init(size_t n_args, const mp_obj_t *args)
{
    // Arguments: horizontal
    if (n_args != 1)
    {
        mp_raise_ValueError(MP_ERROR_TEXT("init requires 1 argument: horizontal"));
    }
    lcd_init(mp_obj_is_true(args[0]));
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(waveshare_lcd_init_obj, 1, 1, waveshare_lcd_init);

// Function to reset the LCD
STATIC mp_obj_t waveshare_lcd_reset(void)
{
    lcd_reset();
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_0(waveshare_lcd_reset_obj, waveshare_lcd_reset);
...
STATIC mp_obj_t waveshare_lcd_blit(size_t n_args, const mp_obj_t *args)
{
    ...
    uint8_t x = mp_obj_get_int(args[0]);
    uint8_t y = mp_obj_get_int(args[1]);
    uint8_t width = mp_obj_get_int(args[2]);
    uint8_t height = mp_obj_get_int(args[3]);
...
{
    // Arguments: x, y, char, color, font_size (optional)
    if (n_args < 4)
    {
...
{
    // Arguments: x, y, text, color, font_size (optional)
    if (n_args < 4)
    {
...
STATIC const mp_rom_map_elem_t waveshare_lcd_module_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_reset), MP_ROM_PTR(&waveshare_lcd_reset_obj)},
