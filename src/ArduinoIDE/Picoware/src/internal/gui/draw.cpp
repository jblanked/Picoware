...
#define TFT_HEIGHT picoBoard.height
    ...
    void Draw::imagePGM(Vector position, const uint8_t *bitmapPGM, Vector size, const uint16_t *palette, bool imageCheck)
    {
        if (!imageCheck || (imageCheck &&
                            bitmapPGM != nullptr &&
                            position.x < this->size.x &&
                            ...
                            size.y > 0))
        {
                ...
                if (!this->useTFT)
                {
                    ...
                    for (int y = 0; y < size.y; y++)
                    {
                        for (int x = 0; x < size.x; x++)
                        {
                            ...
                            uint16_t color = palette != nullptr ? palette[pixel] : this->getPalette()[pixel];
            ...
            else
            {
                ...
                for (int y = 0; y < size.y; y++)
                {
                    for (int x = 0; x < size.x; x++)
                    {
                        uint16_t pixel = pgm_read_word_near(&bitmapPGM[static_cast<int>(y * size.x + x) * 2]);
