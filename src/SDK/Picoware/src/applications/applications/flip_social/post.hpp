...
static void flipSocialPostStop(ViewManager *viewManager)
{
    ...
    if (flipSocialPostSaveRequested && viewManager->getKeyboard() != nullptr)
    {
        ...
        if (response.length() == 0 || response.find("ERROR") != -1)
        {
...
static const View flipSocialPostView = View("FlipSocialPost", flipSocialPostRun, flipSocialPostStart, flipSocialPostStop);