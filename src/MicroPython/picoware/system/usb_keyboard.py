...
class USBKeyboard:
    ...
    def __init__(self, manufacturer='MicroPython', product='Pico Keyboard', serial='000000'):
    ...
    def init(self):
        ...
        hid_iface = bytes([
            ...
            0x09, 0x21, 0x11, 0x01, 0x00, 0x01, 0x22, len(self.HID_REPORT_DESC), 0x00,
        ...
        usbdev.config(
            ...
            desc_strs={0: None, 1: self._manufacturer, 2: self._product, 3: self._serial},
            open_itf_cb=None,
            reset_cb=None,
    ...
    def _on_control_xfer_cb(self, stage, request):
        ...
        if bmRequestType == 0x81 and bRequest == 0x06 and (wValue >> 8) == 0x22 and wIndex == 2:
            return self.HID_REPORT_DESC[:wLength]

        if bmRequestType == 0x21 and bRequest in (0x0A, 0x0B):
            if stage in (1, 3): return True
