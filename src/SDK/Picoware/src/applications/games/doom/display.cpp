...
void drawText(int16_t x, int16_t y, uint16_t num, Draw *const canvas, uint16_t color)
{
    char buf[4];
    snprintf(buf, 4, "%d", num);
    canvas->text(Vector(x, y), buf, color);
}
...
{
    // Use the new imageBitmap method for 1-bit packed bitmaps
    canvas->imageBitmap(Vector(x, y), i, Vector(w, h), color, false);
}
