/*
Author: JBlanked
License: GPL-3.0 License
Source: https://github.com/jblanked/Picoware
*/

#pragma once

#ifdef __cplusplus
extern "C"
{
#endif

#include "py/runtime.h"
#include "py/obj.h"
#include "py/objarray.h"
#include "py/mphal.h"

    typedef struct
    {
        mp_obj_base_t base;
        bool is_initialized;
    } c_mp_obj_t;

    extern const mp_obj_type_t c_mp_type;

    void c_mp_print(const mp_print_t *print, mp_obj_t self_in,
                    mp_print_kind_t kind);
    mp_obj_t c_mp_make_new(const mp_obj_type_t *type, size_t n_args,
                           size_t n_kw, const mp_obj_t *args);
    mp_obj_t c_mp_del(mp_obj_t self_in);
    void c_mp_attr(mp_obj_t self_in, qstr attribute, mp_obj_t *destination);
    mp_obj_t c_mp_exec(mp_obj_t self_in, mp_obj_t path);
    mp_obj_t c_mp_run(mp_obj_t self_in, mp_obj_t source);

#ifdef __cplusplus
}
#endif