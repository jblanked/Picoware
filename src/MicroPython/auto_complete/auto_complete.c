...
static void collect_words(TrieNode *node, AutoComplete *context)
{
    ...
    if (node->is_end_of_word && node->word)
    {
        ...
        char **new_suggestions = (char **)m_realloc(context->suggestions,
                                                    (context->suggestion_count + 1) * sizeof(char *));
        if (!new_suggestions)
            return;

        context->suggestions = new_suggestions;
        context->suggestions[context->suggestion_count] = (char *)m_malloc(len + 1);
...
{
    TrieNode *node = (TrieNode *)m_malloc(sizeof(TrieNode));
    if (!node)
        return NULL;
...
bool auto_complete_add_word(AutoComplete *context, const char *word)
{
    ...
    current->word = (char *)m_malloc(len + 1);
    if (current->word)
    {
    ...
    if (context && context->root)
    {
        ...
        if (context->suggestions)
        {
            for (uint8_t i = 0; i < context->suggestion_count; i++)
            {
                ...
                {
                    m_free(context->suggestions[i]);
                    context->suggestions[i] = NULL;
                }
            }
            m_free(context->suggestions);
