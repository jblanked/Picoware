...
STATIC mp_obj_t auto_complete_mp_free(mp_obj_t self_in)
{
    auto_complete_mp_obj_t *self = MP_OBJ_TO_PTR(self_in);
    auto_complete_free(&self->context);
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(auto_complete_mp_free_obj, auto_complete_mp_free);
...
STATIC mp_obj_t auto_complete_mp_search(mp_obj_t self_in, mp_obj_t prefix_obj)
{
    ...
    if (auto_complete_search(&self->context, prefix))
    {
