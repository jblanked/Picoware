...
class Editor:
            ...
            if line == self.total_lines:
                ...
                if c < Editor.height - 1:
                    ...
                    l = (
                        flag,
                        self.content[line][
                            self.margin : self.margin + Editor.width - 1
                        ],
                    )
    ...
    def line_edit(self, prompt, default, zap=None):
        ...
        while True:
            ...
            if key == KEY_NONE:
                ...
                if (
                    char[1] < Editor.height
                    and (char[1] + self.top_line) < self.total_lines
                ):
    ...
    def find_in_file(self, pattern, col, end):
        ...
        try:
            ...
            return None
        start = self.cur_line
        if col > len(self.content[start]) or (pattern[0] == "^" and col != 0):
            start, col = start + 1, 0
    ...
    def handle_edit_keys(self, key, char):
        ...
        if key == KEY_NONE:
            ...
            if char[2] == 0x22:
                ...
                if (col, line) == self.mouse_last[:2] and (
                    time.time() - self.mouse_last[2]
                ) < 2:
                    self.mouse_last = (0, 0, 0)
                    if (
                        self.mark is None
                    ...
                    if self.mark is not None:
                        if (
                            self.mark_order(self.cur_line, self.col)
                            * self.mark_order(line, col)
                            < 0
                        ):
                            self.mark = self.cur_line, self.col
        ...
        if key == KEY_DOWN:
            ...
            if self.mark is not None:
                ...
                self.undo_add(
                    self.cur_line, [l, self.content[self.cur_line + 1]], KEY_NONE
                )
            ...
            if self.mark is not None:
                ...
                self.undo_add(
                    self.cur_line - 1, [self.content[self.cur_line - 1], l], KEY_NONE
                )
                self.col = len(self.content[self.cur_line - 1])
            ...
            if self.cur_line < (self.total_lines - 1):
                self.undo_add(
                    self.cur_line, [l, self.content[self.cur_line + 1]], KEY_NONE, 1
                )
            else:
            ...
            if self.col < len(l):
                ...
                if i >= 0:
                    ...
                    self.message = "No match in {} lines".format(
                        abs(lstop - self.cur_line)
                    )
        elif key == KEY_MARK:
            if self.mark is None:
                self.set_mark()
            ...
            if start_line > 0:
                ...
                (
                    self.content[start_line - 1 : end_line - 1],
                    self.content[end_line - 1],
                ) = (
            ...
            fname = self.line_edit(
                "Save File: ",
                self.fname if self.is_dir is False else "",
                Editor.file_char,
            )
            if fname:
                if fname != self.fname:
                    # Check if file exists using storage system
                    if self.storage:
                        # Try to read the file to check if it exists
                        existing_content = self.storage.read(fname, "r")
                        if existing_content:  # File exists
                            res = self.line_edit(
                                "The file exists! Overwrite (y/N)? ", "N"
                            )
            ...
            for i in range(lrange[0], lrange[1]):
                if self.content[i].strip() != "":
                    ...
                    if self.content[i][ns : ns + ni] == Editor.comment_char:
                        ...
                        self.content[i] = (
                            ns * " " + Editor.comment_char + self.content[i][ns:]
                        )
        elif key == KEY_REDRAW:
            self.redraw(True)
        elif key == KEY_PLACE:
            ...
            if len(self.undo) > 0:
                self.undo_index = (
                    self.undo_index + (1 if key == KEY_UNDO_NEXT else -1)
                ) % len(self.undo)
                ...
                if self.hash != self.hash_buffer():
                    ...
                    if res and res[0].upper() == "Y":
                        ...
                        if not fname:
                            ...
                            fname = self.line_edit("Save as: ", "", Editor.file_char)
                            if not fname:
                                continue
                        ...
                        try:
                            ...
                            if not error_res or error_res[0].upper() != "Y":
                                continue
                    elif res and res[0].upper() == "F":
                        key = KEY_FORCE_QUIT
                    elif not res or res[0].upper() == "N":
                        # Continue with quit without saving
                        pass
                    else:
                        continue

                Editor.place_list = [
                    item for item in Editor.place_list if item[1] != self
                ]
        ...
        if fname:
            ...
            try:
                if fname in (".", "..") or (os.stat(fname)[0] & 0x4000):
                    ...
                    self.work_dir = os.getcwd()
                    self.fname = (
                        "/" if self.work_dir == "/" else self.work_dir.split("/")[-1]
                    )
                    self.content = [
                        "Directory '{}'".format(self.work_dir),
                        "",
                    ] + sorted(os.listdir("."))
                    self.is_dir = True
                    self.hash = self.hash_buffer()
                ...
                # Not a directory or doesn't exist, treat as file
                pass

            # Handle as file
            # Use storage system for reading files
            if self.storage:
                file_content = self.storage.read(fname, "r")
                if file_content:
                    self.content = file_content.splitlines()
                else:
                    # File doesn't exist, create empty content for new file and write it
                    self.content = [""]
                    # Create the file on SD card so it exists
                    self.storage.write(fname, "\n", "w")
            else:
                # Fallback to direct file operations if no storage
                try:
                    if is_micropython:
            ...
            for l in self.content:
                self.content[i] = self.expandtabs(
                    l.rstrip() if isinstance(l, str) else l
                )
                i += 1
            ...
            try:
                ...
                for l in self.content:
                    if self.write_tabs == "y":
                        file_content += self.packtabs(l)
                    else:
                        file_content += l
                    file_content += "\n"

                # Write using storage system
                success = self.storage.write(fname, file_content, "w")

                if not success:
                    raise OSError(f"Failed to write file {fname}")

                # Force sync/flush if storage has these methods
                ...
                try:
                    written_content = self.storage.read(fname, "r")
                    if not written_content:
                        raise OSError(f"File {fname} appears empty after write")
                    if abs(len(written_content) - len(file_content)) > 10:
                        raise OSError(f"File {fname} size mismatch after write")
                except Exception as e:
                    # If we can't verify, at least log the issue but don't fail
                    print(f"Warning: Could not verify file write for {fname}: {e}")

            except Exception as e:
                raise OSError(f"Error writing file {fname}: {e}")
        else:
            # Fallback to direct file operations if no storage
...
def pye_edit(content, tab_size=4, undo=50, io_device=None, storage=None):
    ...
    if content:
        slot = []
        for f in content:
            slot.append(Editor(tab_size, undo, io_device, storage))
            if type(f) == str and f:
                try:
        ...
        slot = [Editor(tab_size, undo, io_device, storage)]
